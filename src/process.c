...
static int read_system_uptime(double *out)
{
	FILE *fp;
	double uptime;
...
int process_read_list(process_list_t *out, process_sort_mode_t sort_mode, int limit)
{
	...
	if (read_system_uptime(&uptime_sec) != 0 || uptime_sec <= 0.0) {
		return -1;
	}
	...
	while ((ent = readdir(dir)) != NULL && out->count < MAX_PROCESSES) {
		...
		dst->pid = pid;
		dst->mem_bytes = rss;
		dst->cpu_pct = 100.0 * ((double)proc_ticks / (double)clk_tck) / uptime_sec;
		if (dst->cpu_pct < 0.0) {
			dst->cpu_pct = 0.0;
		}
		out->count++;
